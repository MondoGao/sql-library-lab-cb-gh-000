...
def select_name_and_series_subgenres_of_authors
  "SELECT authors.name, subgenres.name FROM subgenres JOIN series ON series.subgenre_id = subgenres.id JOIN authors ON authors.id = series.author_id"
end

def select_series_title_with_most_human_characters
  "SELECT series.title FROM series WHERE series.id = (SELECT series_id FROM characters WHERE species = 'human' GROUP BY series_id ORDER BY COUNT(id) DESC LIMIT 1)"
end
